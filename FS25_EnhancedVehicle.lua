...
function FS25_EnhancedVehicle:new(mission, modDirectory, modName, i18n, gui, inputManager, messageCenter)
  ...
  -- Client resources are created only after the mission HUD and GUI exist.
  -- Early dedicated-process detection prevents headless resource allocation.
...
function FS25_EnhancedVehicle:delete()
  ...
  -- Release mod-owned GUI, scene, and audio resources before mission teardown.
...
function FS25_EnhancedVehicle:onMissionLoaded(mission)
  ...
  -- Dedicated missions must never construct client HUD, GUI, renderer, or audio resources.
  ...
  -- Persistent scene geometry remains visible under anti-aliasing modes that
  -- suppress the engine's debug-line primitives.
...
function FS25_EnhancedVehicle.isDedicatedServerMission(mission)
  ...
  if mission ~= nil and type(mission.getIsServer) == "function" and
    ...
    -- FS25's -server process can report both client and server capability
    -- before g_dedicatedServerInfo exists. The earlier process flag keeps this
    -- headless while hosted and single-player missions retain client access.

    return isServer and (not isClient or isDedicatedProcess)
  end

  -- These globals are fallbacks only when mission capabilities are unavailable.
...
function FS25_EnhancedVehicle:loadMap()
  ...
  -- Dedicated missions use clean in-memory defaults and perform no config file I/O.
  FS25_EnhancedVehicle:resetConfig()
  if lC:getFileAccessAllowed() then
    -- Current configuration is authoritative; legacy data is a one-time fallback.
    local _, _, shouldWriteConfig = lC:readConfig()
    -- New defaults and verified migrations are written only after a successful read.
...
-- EnhancedVehicle and FS25_EnhancedVehicle identifiers are compatibility ABI.
-- Existing saves, mods, bindings, and network peers depend on them.
...
-- Public compatibility API for other mods.
-- Supported names: differential, hydraulic, snap, park, and odometer.
-- State is persisted through the established configuration keys.
...
-- Public compatibility API matching functionEnable.
-- Returns the persisted state for a supported name.
...
function FS25_EnhancedVehicle:activateConfig()
  ...
  -- Sanitize persisted values before exposing them to runtime systems.
...
function FS25_EnhancedVehicle:onPostLoad(savegame)
  ...
  -- vData.is and vData.want are legacy save/network ABI: 1-3 drivetrain,
  -- 4-6 guidance modes, 7-12 track pose, 13 brake, 14-16 odometer state.
  ...
  self.vData.networkThreshold = 10 -- Odometer network update interval in metres.
  self.vData.odoDistanceSent  = 0
  self.vData.tripDistanceSent = 0


  if self.isServer then
    for _, differential in ipairs(self.spec_motorized.differentials) do
      if differential.diffIndex1 == 1 then -- FS25 front differential index.
        self.vData.torqueRatio[1]   = differential.torqueRatio
        self.vData.maxSpeedRatio[1] = differential.maxSpeedRatio
      end
      if differential.diffIndex1 == 3 then -- FS25 rear differential index.
        self.vData.torqueRatio[2]   = differential.torqueRatio
        self.vData.maxSpeedRatio[2] = differential.maxSpeedRatio
      end
      if differential.diffIndex1 == 0 and differential.diffIndex1IsWheel == false then -- FS25 centre differential index.
...
function FS25_EnhancedVehicle.sanitizeNetworkSnapshot(vehicle, incoming, fromClient)
  ...
  if fromClient then
    -- Odometer values are server-authoritative; clients may request only a
    -- narrowly scoped trip reset.
  ...
  -- Legacy slots 7-12 remain canonical with the named track snapshot fields.
...
function FS25_EnhancedVehicle.getGuidanceDirectionSign(vehicle)
  ...
  if reverseSpec ~= nil and reverseSpec.aiSteeringNode == nil and
    ...
    -- ReverseDriving falls back to the forward AI node when no reverse node
    -- exists. Flip only that fallback; a dedicated AI node already represents
    -- the active driving direction.
...
local function getGuidanceGeometryFrame(vehicle, directionNode, directionSign)
  ...
  -- Applying the sign to both horizontal axes represents the same 180-degree
  -- frame used by the reverse-driving fallback.
...
function FS25_EnhancedVehicle.guidanceGeometryNeedsRefresh(vehicle, directionNode, directionSign)
  ...
  -- Avoid allocating a frame table in this per-frame refresh check.
  ...
  if isControlled and g_gui ~= nil and not g_gui:getIsGuiVisible() then
    ...
    if FS25_EnhancedVehicle.functionSnapIsEnabled then
      ...
      if self.vData.opMode == 2 and self.vData.track.isCalculated and _showLines and (FS25_EnhancedVehicle.track.showLines == 1 or FS25_EnhancedVehicle.track.showLines == 3) then
        ...
        self.vData.track.dotFBPrev = dotFB -- Retained for forward/reverse pass detection.
...
function FS25_EnhancedVehicle:onPostDetachImplement(implementIndex)
  ...
  -- Attachment state is server-authoritative and may change while unoccupied
  -- or AI-controlled, so invalidation cannot depend on a client owner.
...
function FS25_EnhancedVehicle.setHydraulicGroupTurnedOn(vehicle, implements, label)
  ...
  for _, object in pairs(implements) do
    if object.spec_turnOnVehicle ~= nil then
      ...
      elseif not object:getCanToggleTurnedOn() then
        -- Always-on and attacher-controlled tools are not manual group members.
  ...
  -- FS25's power-chain contract must allow every member before the group starts.
...
-- Every tool follows its own FS25 Foldable direction, guard, and middle-state contract.
...
function FS25_EnhancedVehicle:onActionCall(actionName, keyStatus, arg4, arg5, arg6)
  ...
  if FS25_EnhancedVehicle.functionSnapIsEnabled then
    ...
    elseif actionName == "FS25_EnhancedVehicle_SNAP_LINES_MODE" then
      -- Cycle hidden, tracks, tracks plus vehicle guides, vehicle guides, then
      -- hidden again while preserving the legacy numeric configuration values.
...
-- FS25 maps uncut grass to FieldGroundType.GRASS; cut grass remains field ground.
...
-- updateAngleValue -1 derives the direction from the vehicle.
-- updatePosition selects a new origin; deltaPosition shifts it laterally.
-- deltaTrack, deltaOffset, and deltaWorkWidth adjust the current pass layout.
-- updateSnap recalculates and broadcasts the target point.
...
function FS25_EnhancedVehicle:updateTrack(self, updateAngle, updateAngleValue, updatePosition, deltaPosition, updateSnap, deltaTrack, deltaOffset, deltaWorkWidth)
  ...
  -- Preserve the equivalent pass when either offset or work width changes.
  ...
  if updateSnap then
    ...
    -- Project the vehicle into the original grid and current driving frames.
    local dotLR = dx * -self.vData.track.origin.originaldZ + dz * self.vData.track.origin.originaldX
  ...
  -- A broadcast snapshot must already advertise a complete, valid layout.
    ...
    if obj.spec_workArea ~= nil and obj.spec_workArea.workAreas ~= nil then
      for _, workArea in pairs(obj.spec_workArea.workAreas) do
        ...
        if functionName ~= nil and
          ...
          -- All work-area nodes are transformed into the vehicle's guidance
          -- frame; the height node matters for skewed or rotated tools.
...
-- Normalize degrees to the half-open interval [0, 360).
...
function FS25_EnhancedVehicle:updateVehiclePhysics(superFunc, axisForward, axisSide, doHandbrake, dt)
  ...
  if FS25_EnhancedVehicle.functionSnapIsEnabled and self.vData ~= nil and self.vData.is[5] then
    if self:getIsVehicleControlledByPlayer() and self:getIsMotorStarted() then
      -- Use the AI direction frame consistently for heading and lateral error.
      local directionNode = FS25_EnhancedVehicle.getGuidanceDirectionNode(self)
      local directionSign = FS25_EnhancedVehicle.getGuidanceDirectionSign(self)
      local px, _, pz = localToWorld(directionNode, 0, 0, 0)
      local lx, _, lz = localDirectionToWorld(directionNode, 0, 0, directionSign)
      ...
      if self.vData.is[6] then
        ...
        if math.abs(dotLR) < 0.05 then dotLR = 0 end -- Suppress sub-centimetre steering jitter.
      end

      -- Track correction runs when either heading or lateral error is non-zero.

      if math.abs(self.vData.rot - self.vData.is[4]) > 0.0001 or dotLR ~= 0 then

        -- Stationary input uses forward direction to keep the controller deterministic.
        ...
        -- Scale steering rate by frame time and effective driving direction.
        local delta = dt/500 * movingDirection

        -- Compare headings in the signed [-180, 180] interval.
        local _w1 = self.vData.is[4]
        if _w1 > 180 then _w1 = _w1 - 360 end
        local _w2 = self.vData.rot

        -- Lateral error bends the desired heading more strongly at low speed.
        if self.vData.is[6] then

          _w2 = _w2 - Between(dotLR * Between(10 - self:getLastSpeed() / 8, 4, 8) * movingDirection * 1.3, -90, 90)
        ...
        -- Outside 18 degrees steering ramps; inside it converges linearly and
        -- snaps directly within 2 degrees.
        local _d = 18
        ...
        if (diffdeg >= -_d) and (diffdeg <= _d) then
          local newa = diffdeg / _d * movingDirection
          if a < newa then
  ...
  -- Use the specialization chain so failures remain visible and other
  -- specializations retain their normal ordering.
...
-- FS25's manual-transmission path bypasses the ordinary parking-brake input,
-- so the wheel-pedal helper is hooked once at module scope.
